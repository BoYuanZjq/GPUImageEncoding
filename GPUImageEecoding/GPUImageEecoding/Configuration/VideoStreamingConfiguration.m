...
@interface VideoStreamingConfiguration()
@property (nonatomic, assign,readwrite) CGSize  videoSize;
@end

@implementation VideoStreamingConfiguration
#pragma mark -- LifeCycle
+ (instancetype)defaultConfiguration{
    VideoStreamingConfiguration *configuration = [VideoStreamingConfiguration defaultConfigurationForQuality:LMStreamingVideoQuality_Default withPosition:LMCaptureDevicePositionFront];
    return configuration;
}

+ (instancetype)defaultConfigurationForQuality:(LMStreamingVideoQuality)videoQuality withPosition:(LMCaptureDevicePosition)position{
    VideoStreamingConfiguration *configuration = [VideoStreamingConfiguration new];
    switch (videoQuality) {
    ...
    if (!position || (position!=LMCaptureDevicePositionFront && position!=LMCaptureDevicePositionBack)) {
        configuration.capturePosition = LMCaptureDevicePositionFront;
    }
    configuration.capturePosition = position;
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    [aCoder encodeObject:@(self.sessionPreset) forKey:@"sessionPreset"];
     [aCoder encodeObject:@(self.capturePosition) forKey:@"capturePosition"];
}
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    _capturePosition = [[aDecoder decodeObjectForKey:@"capturePosition"] unsignedIntegerValue];
    return self;
}
...
- (BOOL)isEqual:(id)other
{
    if (other == self) {
        ...
        VideoStreamingConfiguration *object = other;
        ...
        object.capturePosition == self.capturePosition;
    }
}

- (id)copyWithZone:(nullable NSZone *)zone{
    VideoStreamingConfiguration *other = [self.class defaultConfiguration];
    return other;
}
...
- (NSString *)description{
    ...
    [desc appendFormat:@" capturePosition:%zi",self.capturePosition];
    return desc;
}
